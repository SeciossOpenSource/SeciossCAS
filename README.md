...
### データベースのインストール
クラウドサービス検出用のデータベースを作成します。MariaDBをインストールしてデータベースcasbに以下のテーブルを作成して下さい。
~~~
CREATE TABLE cloud_discovery (
...
~~~
アクティビティログのグラフ用の情報を登録するデータベースを作成します。データベースreportに以下のテーブルを作成して下さい。
~~~
...
## 実行
### CASB
アクティビティログは、以下のコマンドを実行して取得できます。
```
# /opt/secioss/sbin/service_auditlog.pl
```
取得したログは、LISMの管理コンソールの[CASB]-[アクティビティログ]から参照することができます。
コマンドはcronにより定期的に実行するように設定して下さい。
### DLP
